    ...
    return (
      <section className="container mx-auto my-12 max-w-4xl p-4">
        <div className="p-4 text-center">
          <p className="text-sm font-semibold uppercase text-black-900">
          ...
          <p className=" text-sm text-gray-600 ">
            Posso atender desenvolvimentos em FrontEnd.
          </p>
        </div>
  
        <div className="mt-7 flex flex-col gap-3 md:flex-row justify-around">
          {services.map((service, index) => (
            <div
              className="basis-1/3 bg-purple-300 p-4 text-black-900 rounded-md"
              key={`service-${index}`}
            >
